...
class TradingCore:
    ...
    def get_unrealized_pnl(self, current_price):
        ...
        if self.position['type'] == 'LONG': return (current_price - entry) * size
        else: return (entry - current_price) * size
    ...
    def process_step(self, action, row_1h, timestamp, precision_candles=None):
        ...
        if self.position:
            funding_cost = (self.position['entry_price'] * self.position['size']) * self.rules['funding_rate_hourly']
    ...
    def _open_position(self, p_type, price, atr, timestamp):
        ...
        if (size * real_price) < self.rules['min_trade_amount']: return
